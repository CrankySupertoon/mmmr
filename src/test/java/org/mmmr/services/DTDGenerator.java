...
/**
 * DTDGenerator<BR>
 * Generates a possible DTD from an XML document instance. Pure SAX version of the Saxon DTDGenerator The program has no remaining dependencies on
 * Saxon; all it needs is: JAXP 1.1 SAX2 A JAXP 1.1 conformant XML parser Java 1.2
 * 
 * @author M.H.Kay
 * @version 7.0: (Sep 2001) separated from Saxon source, now works with any JAXP 1.1 XML parser
 * @version 7.0.1: (Sep 2007) avoid holding all attributes in memory, correct the -? output message
 */
...
public class DTDGenerator extends org.xml.sax.helpers.DefaultHandler {
    ...
    /**
     * ChildDetails records information about the presence of a child element within its parent element. If the parent element is sequenced, then the
     ...
     *         This static method converts a Unicode string to a string containing only ASCII characters, in which non-ASCII characters are
     *         represented by the usual XML/HTML escape conventions (for example, "&lt;" becomes "&amp;lt;"). Note: if the input consists solely of
     *         ASCII or Latin-1 characters, the output will be equally valid in XML and HTML. Otherwise it will be valid only in XML. The escaped
     *         characters are written to the dest array starting at position 0; the number of positions used is returned as the result
     */
     ...
     *         This static method converts a Unicode string to a string containing only ASCII characters, in which non-ASCII characters are
     *         represented by the usual XML/HTML escape conventions (for example, "&lt;" becomes "&amp;lt;").<br>
     *         Note: if the input consists solely of ASCII or Latin-1 characters, the output will be equally valid in XML and HTML. Otherwise it will
     *         be valid only in XML.
     */
    ...
    /**
     * Test whether a string is an XML NMTOKEN. TODO: This is currently an incomplete test, it treats all non-ASCII characters as being valid in
     * NMTOKENs.
     */
    ...
    private boolean isValidNMTOKEN(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            int c = s.charAt(i);
            if (!(((c >= 0x41) && (c <= 0x5a)) || ((c >= 0x61) && (c <= 0x7a)) || ((c >= 0x30) && (c <= 0x39)) || (c == '.') || (c == '_')
                    || (c == '-') || (c == ':') || (c > 128))) {
                return false;
            }
        }
    ...
    /**
     * Handle the start of an element. Record information about the position of this element relative to its parent, and about the attributes of the
     * element.
     */
    ...
    public void startElement(String uri, String localName, String name, Attributes attributes) throws SAXException {
        ...
        for (int a = 0; a < attributes.getLength(); a++) {
            ...
            if (!ad.values.contains(val)) {
                ...
                // ad.values.add(val); - line removed in 7.0.1
                ...
                if (ad.unique && ad.allNames && (ad.occurrences <= DTDGenerator.MAX_ID_VALUES)) {
                    ad.values.add(val);
                } else if (ad.values.size() <= DTDGenerator.MAX_ENUMERATION_VALUES) {
                    ad.values.add(val);
                }
    ...
    public void write(PrintStream pw) {
        ...
        while (e.hasNext()) {
            ...
            if ((childKeys.size() == 0) && !ed.hasCharacterContent) {
                pw.print("<!ELEMENT " + elementname + " EMPTY >\n");
            }

            // CHARACTER content
            if ((childKeys.size() == 0) && ed.hasCharacterContent) {
                pw.print("<!ELEMENT " + elementname + " ( #PCDATA ) >\n");
            }

            // ELEMENT content
            if ((childKeys.size() > 0) && !ed.hasCharacterContent) {
                pw.print("<!ELEMENT " + elementname + " ( ");
            ...
            while (a.hasNext()) {
                ...
                boolean isid = ad.allNames && // ID values must be Names
                        ...
                        (ad.unique) && (ad.occurrences >= DTDGenerator.MIN_ID_VALUES);

                // if there is only one attribute value, and at least MIN_FIXED occurrences of it,
                // treat it as FIXED
                boolean isfixed = required && (ad.values.size() == 1) && (ad.occurrences >= DTDGenerator.MIN_FIXED);
                ...
                boolean isenum = ad.allNMTOKENs
                        ...
                        (ad.occurrences >= DTDGenerator.MIN_ENUMERATION_INSTANCES)
                        && (ad.values.size() <= ad.occurrences / DTDGenerator.MIN_ENUMERATION_RATIO)
                        && (ad.values.size() <= DTDGenerator.MAX_ENUMERATION_VALUES);

                pw.print("<!ATTLIST " + elementname + " " + attname + " ");
                ...
                if (isid) {
                    ...
                    pw.print(tokentype + " #FIXED \"" + DTDGenerator.escape(val) + "\" >\n");
                } else if (isenum) {
                    pw.print("( ");
                    Iterator v = ad.values.iterator();
                    while (v.hasNext()) {
                        pw.print((String) v.next());
                        if (!v.hasNext()) {
                            break;
                        }
                        pw.print(" | ");
