...
public class BatCheck {
    public static void check(Config cfg) throws IOException {
        ...
        if (!noconsole.exists()) {
            ...
            byte[] bytes = batstring.replaceAll("\\Q{CONSOLE}\\E", "w").replaceAll("\\Q{JAR}\\E", IOMethods.getCurrentJar().getName()).getBytes(); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            FileOutputStream bnco = new FileOutputStream(noconsole);
            bnco.write(bytes);
            bnco.close();
        }
        if (!console.exists()) {
            byte[] bytes = batstring.replaceAll("\\Q{CONSOLE}\\E", "").replaceAll("\\Q{JAR}\\E", IOMethods.getCurrentJar().getName()).getBytes(); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        ...
        if (cfg.getParameterValue("console") == null) { //$NON-NLS-1$
            IOMethods.showInformation(cfg, Messages.getString("BatCheck.runningMMMR"), //$NON-NLS-1$
    ...
    public static void force(File oldjar, File newjar) throws IOException {
        ...
        String batstring = "@if exist " + oldjar.getName() + " del " + oldjar.getName() + "\r\n" //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                + new String(IOMethods.read(MMMRStart.class.getClassLoader().getResourceAsStream("bat/start MMMR.bat"))); //$NON-NLS-1$
        {
            // w is put behind java so if starts no console
